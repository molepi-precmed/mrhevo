...
get_estimatorsMR <- function(coeffs) {
    coeffs[, weights := se.thetaIV^-2]  
    ## IVW estimate and SE
    thetaIVW <- coeffs[, sum(thetaIV * weights) / sum(weights)] 
...
#' @param priorsd_theta Standard deviation of prior on theta.
#' @returns An object of class stanfit. 
run_mrhevo.sstats <- function(N, alpha_hat, se.alpha_hat, gamma_hat, se.gamma_hat, fraction_pleio=NULL, priorsd_theta=1) {
    require(rstan)
    options(mc.cores = parallel::detectCores())
    rstan_options(auto_write = TRUE)
    ...
    if(logistic) {
        mu <- mean(Y)
        pseudoSE <- (mu * (1 - mu))^-0.5 / sqrt(N)
    } else {
        pseudoSE <- sigma_y / sqrt(N)
    }
...
#' get typical value tau0 for global shrinkage parameter tau for horseshoe prior
...
set.tau0 <- function(fraction_pleio=NULL, nu_global=1, J, pseudoSE=pseudoSE) {
    if(is.null(fraction_pleio)) {
        fraction_pleio <- 0.5 # prior guess of number of instruments that are pleiotropic
    }
    r_pleio <- fraction_pleio * J
    ...
    tau0 <- (r_pleio / (J - r_pleio)) * pseudoSE
    ## choose prior on tau so that most of the prior mass is near tau_0
    ## choose scale_global so that the prior median of the half-t distribution equates to tau_0
    return(tau0)
}
